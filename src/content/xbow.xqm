...
(:~
 ...
 : <code>true()</code>
 :)
declare function xbow:all (
	$sequence as item()*,
	$comparison-function as function(*)
) as xs:boolean {
	fold-left(
		$sequence,
		true(),
...
declare function xbow:pluck (
	$map-or-array-or-node as item(),
	$field as xs:anyAtomicType
) as item()* {
...
declare %private function xbow:pluck-node-part (
	$node as node(),
	$path-part as xs:string?
) as node()? {
...
declare function xbow:pluck-deep (
	$map-or-array-or-node as item(),
	$fields as xs:anyAtomicType*
) as item()* {
	fold-left(
		$fields,
...
declare function xbow:disdup-reducer ($result as map(*), $next as xs:string) as map(*) {
	if (exists($result?distinct($next))) then (
		map:put($result, 'duplicates', ($result?duplicates, $next))
	) else (
...
declare variable $xbow:initial-stats := map {
	'min': (),
	'max': (),
	'avg': 0.0,
	'sum': 0,
...
(:~
 ...
 :)
declare function xbow:wrap-element ($value, $name) {
	element {$name} { $value }
};
...
(:~
 ...
 :)
declare function xbow:wrap-attribute (
	$value as item()*,
...
declare function xbow:wrap-attribute ($value as item()*, $attribute-name as xs:string) {
...
(:~
 ...
 : )
 :)
declare function xbow:wrap-each ($values, $node-name as xs:string) {
	for-each($values, xbow:wrap-element(?, $node-name))
};

declare function xbow:wrap-map-attribute ($map as map(*)) {
	map:for-each($map, function ($k, $v) { attribute {$k} { $v } })
};

declare function xbow:wrap-map-element ($map as map(*)) {
	map:for-each($map, function ($k, $v) { element {$k} { $v } })
};

declare function xbow:map-filter-keys ($map as map(*), $keys as xs:string*) {
	let $f := function ($key) { map:entry($key, $map($key)) }

	return for-each($keys, $f) => map:merge()
};
...
declare function xbow:map-flip ($map as map(*)) as map(*) {
	map:for-each(
		$map,
		function ($key as xs:anyAtomicType, $value as xs:anyAtomicType) as map(*) { map {$value: $key} }
	)
		=> map:merge()
};
...
declare %private function xbow:subtype-reducer (
	$result as xs:string?,
	$item as item()?
) as xs:string {
	if ($result = '*') then (
...
declare function xbow:combine ($functions as function(*)*) as function(*) {
	function ($initial as item()*) { fold-left($functions, $initial, xbow:combination-reducer#2) }
};

declare %private function xbow:combination-reducer (
	$result as item()*,
	$next as function(*)
) as item()* {
	...
	return fold-left(
			...
			function ($result as array(*), $item as item()) {
				...
				return if ($pos eq 0) then (
						...
						(: $item did not match any criteria :)
						xbow:array-put($result, $pos, ($result($pos), $item))
					)
			}
		)
};
...
(:~
 ...
 :)
declare function xbow:array-put (
	$array as array(*),
	$pos as xs:integer,
	$items-to-put as item()*
) as array(*) {
	array:join(
		(
			if (array:size($array) < $pos) then (
				$array, xbow:array-fill($pos - array:size($array) - 1, ()), [$items-to-put]
...
declare function xbow:array-fill ($size as xs:integer, $function-or-value as item()?) as array(*) {
	if ($size < 1) then (
		...
		typeswitch ($function-or-value)
			case function(*) return
				xbow:array-for-each-index(array { (1 to $size) }, $function-or-value)

			default (: we need an array to iterate over for [(),()] to be possible :) return
				array:for-each(array { (1 to $size) }, function ($ignore) { $function-or-value })
	)
};
...
declare %private function xbow:find-first-matching (
	$rules as array(*),
	$item as item()
) as xs:integer {
...
declare %private function xbow:assign-label (
	$items as item()*,
	$label as xs:string

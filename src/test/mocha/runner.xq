...
declare function local:get-module-uri ($lib as xs:string, $version as xs:string) as xs:anyURI {
	xs:anyURI(``[xmldb:///db/system/repo/`{$lib}`-`{$version}`/test/`{$lib}`-spec.xqm]``)
};

declare function local:result-to-map ($result as element()) as map(*) {
	map {
		'result':
			map {
...
try {
	let $result := local:get-module-uri($local:lib, $local:version)
		=> inspect:module-functions()
		=> test:suite()
		=> local:result-to-map()

	return ($result, util:log('info', $result))
} catch * {
	map {
		'error':
			map {

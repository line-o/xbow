...
declare variable $xbow-spec:nested-array := [map {'n': '1'}, map {'n': '10'}, map {'n': '11'}, map {'n': '-1'}, map {
	'n': '0'
}, map {'n': '16'}, map {'n': '8'}, map {'n': '31'}, map {'n': '9'}];
...
declare
	variable $xbow-spec:user-xml := <root>
...
declare %test:assertEquals(1) function xbow-spec:pluck-deep () {
	map {'a': map {'b': map {'c': map {'d': map {'e': map {'f': 1}}}}}} => xbow:pluck-deep(('a', 'b', 'c', 'd', 'e', 'f'))
};

declare %test:assertEquals(1) function xbow-spec:pluck-deep-array-in-map () {
	map {'a': ['1', map {'c': map {'d': map {'e': map {'f': 1}}}}]} => xbow:pluck-deep(('a', 2, 'c', 'd', 'e', 'f'))
};
...
declare %test:assertEquals(9) function xbow-spec:distinct-count () {
	$xbow-spec:xml/a/@n/string() => xbow:distinct-duplicates() => xbow:pluck('distinct') => map:keys() => count()
};

declare %test:assertEqualsPermutation('23', '16', '11', '3', '2', '1', '10', '6', '-1')
...
declare %test:assertEquals('1') function xbow-spec:duplicate-values () {
	$xbow-spec:xml/a/@n/string() => xbow:distinct-duplicates() => xbow:pluck('duplicates') => string-join()
};
...
declare %test:assertEqualsPermutation('11', '-1', '16', '0', '1', '8', '9', '31', '10')
...
declare %test:assertEquals('-1', '0', '1', '8', '9', '10', '11', '16', '31') function xbow-spec:map-flip () {
	$xbow-spec:map => xbow:map-flip() => map:keys() => xbow:ascending()
};

declare %test:assertEquals('-2', '-1', '0', '7', '8', '9', '10', '15', '30') function xbow-spec:map-flip-function-add (
...
declare %test:assertEquals('1f61f08a-04d5-3a9b-a749-7fad4d9b612c') function xbow-spec:map-flip-function-uuid () {
...
declare %test:assertEquals('b') function xbow-spec:items-at-single () as item()* {
	xbow:items-at(('a', 'b'), 2)
};

declare %test:assertEquals(3, 1, 9) function xbow-spec:items-at-multiple () as item()* {
	xbow:items-at((1 to 9), (3, 1, 9))
};

declare %test:assertEmpty function xbow-spec:items-at-no-index () as item()* {
	xbow:items-at((1 to 9), ())
};

declare %test:assertEmpty function xbow-spec:items-at-empty-sequence () as item()* {
	xbow:items-at((), (3, 1, 9))
};

declare %test:assertEmpty function xbow-spec:test-exists-empty () as item()* {
	xbow:test((), exists#1, concat('"', ?, '" existed'))
};

declare %test:assertEquals('"node" existed') function xbow-spec:test-exists-node () as item()* {
	xbow:test(<node>node</node>, exists#1, concat('"', ?, '" existed'))
};

declare %test:assertEquals(11) function xbow-spec:test-exists-sequence () as item()* {

...
declare variable $local:age-category-rules := [xbow:lt(12), xbow:lt(20), xbow:lt(30), xbow:lt(60), xbow:lt(120),
xbow:ge(120)];
...
declare function local:render-stats ($map, $seq) {
	for-each(
		$seq,
		function ($key) {
			xbow:wrap-element($key, 'dt'), $map($key) => xbow:wrap-each('dd')
		}
	)
};
